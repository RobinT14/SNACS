...
def calculate_kendall_tau_errors(exact_betweenness, approx_betweenness):
    ...
    approx_values = [approx_betweenness[str(
        int(float(key)))] for key in exact_keys]
...
def main():
    exact_file_path = 'NetworkX_Results/07-12-2023_networkx_exact.json'
    exact_betweenness = read_betweenness_from_json(exact_file_path)

    file1 = open('networkit_output_tuned.log', 'r')
    ...
    for line in Lines:
        ...
        if 'Approximation' in line:
            ...
            tau, mae, rmse = calculate_kendall_tau_errors(
                exact_betweenness, approx_betweenness)
            printLine += f'{tau},{mae},{rmse}'
        print(printLine)
